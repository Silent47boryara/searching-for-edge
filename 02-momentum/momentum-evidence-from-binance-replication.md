...
- significance — three tests, each with its own model of data dependence: a weekly bootstrap confidence interval, a weekly sign-flip test (removes the common crypto beta via sign reversal), and a within-week permutation test (reshuffles coins between the two legs within the same week, preserving basket sizes).
