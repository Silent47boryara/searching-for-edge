...
This makes what happened next more weighty than if J=2/K=2 had just been a "second attempt": two independent sources pointed specifically to the 2-week formation horizon as stronger and more robust — and even it didn't survive the check on our universe.
...
This doesn't mean momentum as a phenomenon is a myth, or that a more elaborate implementation (longer horizons, a narrower liquid universe, a separately tested regime overlay like Starkiller's) couldn't show a different result. It means that a deliberately constrained implementation of an academic momentum hypothesis — Binance perpetuals, equal-weighted baskets, a fixed liquidity threshold, realistic costs, and our own pre-registered J/K protocol — on a real, tradable universe of instruments is not a free lunch you can lift off a paper's page and run as-is.
